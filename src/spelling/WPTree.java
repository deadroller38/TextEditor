...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		while (!queue.isEmpty()) {
			...
			List<String> nearest = nw.distanceOne(curr.getWord(), true);
			for (int i = 0; i < nearest.size(); i++) {
				if (!visited.contains(nearest.get(i))) {
					visited.add(nearest.get(i));
					WPTreeNode child = new WPTreeNode(nearest.get(i), curr);
					queue.add(child);
					if (nearest.get(i).equals(word2)) {
